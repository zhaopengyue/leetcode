package leetcode.editor.cn.t_071_简化路径
...
/**
 * 解答成功:
 * 执行耗时:544 ms,击败了33.33% 的Scala用户
 * 内存消耗:56.1 MB,击败了55.56% 的Scala用户
 */
...
object Solution {
    def simplifyPath(path: String): String = {
        ...
        val pathArr = path.stripPrefix("/").stripSuffix("/").split("/", -1)
        pathArr.foreach {
            case "." => // pass
            case "" => // pass
        ...
        val otherPath = stack.reverse.mkString("/")
        s"/$otherPath"
    }
}
